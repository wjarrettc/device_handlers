/**
 *  Osram Lightify RGBW A19/BR30 US version (HA) DTH with ActionTiles workaround
 *  Based on orginal OSRAM RGBW DH written by gkl-sf with modifications to make ActionTiles recognize the device as a color capable bulb
 *
...
metadata {
    definition (name: "Osram RGBW ActionTiles Patch", namespace: "wjarrettc", author: "wjarrettc") {
    ...
    tiles(scale: 2) {
        multiAttributeTile(name:"switch", type: "lighting", width: 6, height: 4, canChangeIcon: true) {
            ...
            tileAttribute ("device.switch", key: "PRIMARY_CONTROL") {
                ...
                attributeState "turningOn", label:'Turning...', action:"on", icon:"st.lights.philips.hue-single", backgroundColor:"#00a0dc", nextState:"turningOn"
                attributeState "turningOff", label:'Turning...', action:"off", icon:"st.lights.philips.hue-single", backgroundColor:"#ffffff", nextState:"turningOff"
            }           
...
def setColor(value) {
    ...
    cmds << sendEvent(name: "color", value: value.hex, displayed: false)
...
def setHue(value) { //accepts hue values 0-100, doesn't change saturation
    log.debug "setHue acting on  ${value}"
    def cmds = []
    if (!state.colorTransition) state.colorTransition = DEFAULT_COLOR_TRANSITION    
...
def setHue360(value) { //accepts hue values 0-360 and sets saturation to 100%
    log.debug "setHue360 acting on  ${value}"
    if (!state.colorTransition) state.colorTransition = DEFAULT_COLOR_TRANSITION    
...
def setSaturation(value) {
    log.debug "setSaturation acting on  ${value}"
    if (!state.colorTransition) state.colorTransition = DEFAULT_COLOR_TRANSITION    
